...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld)
        payable
        ERC721("Automated Name Service", "NNS")
    {
        tld = _tld;
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 2);
        if (len == 3) {
            return 5 * 10**17; // 0.5 MATIC = 500 000 000 000 000 000 (18 decimals). We're going with 0.5 Matic cause the faucets don't give a lot
        } else if (len == 4) {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the Automated Name Service", "image": "data:image/svg+xml;base64,',
